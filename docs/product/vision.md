...
Version 0.1.1 supplies the local voice vertical slice: supervised loopback llama.cpp/Qwen3 inference, sherpa-onnx wake-word/VAD/ASR/TTS, Windows audio, streaming segmentation, barge-in, push-to-talk, and text debugging. There is no computer control, project intelligence implementation, persistent memory, GUI automation, or IoT adapter yet. See [the roadmap](roadmap.md) for staged scope.
