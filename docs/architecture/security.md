...
Repository excerpts, transcripts, and answers remain untrusted. Strict local-model JSON is bounded, unknown fields are rejected, and one malformed-output repair cannot invoke a tool. Project corrections are facts only when trusted evidence indexes map to current Project Intelligence records. Weak-answer corrections are retained for evaluation but withheld from the immediate targeted-follow-up result.
