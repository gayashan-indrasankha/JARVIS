...
SQLite operations use parameterized values. FTS query syntax is built from at most eight bounded alphanumeric/underscore tokens; arbitrary query operators are not passed through. Transactions replace one repository snapshot atomically. Retrieval caps candidates, traversal depth, excerpts, total evidence characters, tool result characters, and local-model context. Before evidence leaves the trusted index, its row hash must match snapshot metadata and the current approved file is decoded and hashed again; stale, replaced, reparse-routed, or corrupt evidence fails closed. Returned source is explicitly untrusted and every project fact must cite an actual indexed file/line/hash/snapshot. Non-fatal repository-analysis and watcher-refresh exceptions cross the tool boundary only as stable categories so they cannot bypass the terminal audit or fault shutdown with private exception detail.

## Project learning data and model profiles

Project Tutor/Interviewer operations are typed `SAFE_LOCAL_ACTION` tools because they persist private session state and can replace the local managed model process. Validation and repository normalization occur before authorization. The learning service has no filesystem, process, network, Roslyn, SQLite, or Windows authority in Core; Infrastructure supplies evidence only through Project Intelligence and local inference only through provider-neutral ports.

Repository excerpts, transcripts, answers, and learning-model prose remain untrusted. Strict local-model JSON is bounded, unknown fields are rejected, and one malformed-output repair cannot invoke a tool. Scored expected concepts and project-correction text come from mapped Project Intelligence claims rather than model prose. Weak-answer corrections, gaps, and detailed evaluation data are retained for the report but withheld from the immediate targeted-follow-up result; trusted Core supplies the neutral challenge text.

Learning persistence at `JARVIS_HOME\Data\ProjectLearning` contains repository paths, transcripts, scores, weaknesses, and evidence-derived text. It never belongs in Git or logs. Retention and payloads are bounded; persistence can be disabled. Profile logs contain only FAST/DEEP and sanitized reason codes. Optional DEEP uses the same fixed-loopback managed supervisor, never a second simultaneous server or cloud fallback, and falls back to FAST on resource/health failure.

## Verification

Automated architecture tests reject outer/model/native/network/Roslyn/SQLite dependencies from Core and reject tool dispatcher/authorization/audit/project-index references from model adapters. Tool tests cover closed schemas, unknown/malformed proposals, validation/authorization order, denial, approved-root/credential/reparse defenses, fixed command mappings, duplicate calls, cancellation, timeout, truncation, audit completeness, prompt-injection labeling, and temporary-directory behavior. Project tests prove safe static loading, inert hostile targets, DTD denial, exclusions, exact/hash-current evidence, incremental snapshots, multi-hop depth limits, retrieval/context limits, cancellation, disabled configuration, and watcher debounce/failure containment. Configuration/path/loopback tests reject unsafe values. Supervisor and voice tests cover lifecycle, fallback, cancellation, barge-in, and stale output. Repository gates scan for secrets, external endpoint remnants, tracked model/runtime artifacts, and warnings.
