...
The model identifies demonstrated/incorrect expected-concept indexes and bounded qualitative flags. Expected concepts and correction text are derived by trusted Core code from the mapped Project Intelligence facts; model-supplied concept/correction prose never becomes a trusted project fact. Trusted Core code converts those signals into deterministic 0–4 scores for project factual accuracy, depth, reasoning, trade-offs, C#/.NET, database, testing, security, communication, and confidence calibration. Wording is not compared literally. Each score retains its rubric and content-free rationale; the question retains relevant evidence and expected concepts; the session retains transcript, strengths, gaps, and corrections. During a targeted follow-up, corrections, gaps, and detailed stored evaluation data are withheld from the immediate result and a trusted neutral challenge is used so model prose cannot reveal the answer early.
